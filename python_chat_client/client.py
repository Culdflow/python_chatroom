...
def send(msg, encrypted = True):
	try:
		...
		print(f"{Fore.RED}[UKNOWN ERROR]Quitting...{Fore.WHITE}")
		signal.signal(signal.SIGINT, exit_gracefully)
		exit()
...
def receive(encrypted = True):
	try:
		...
		print(f"{Fore.RED}[UKNOWN ERROR]Nothing received{Fore.WHITE}")
		print(f"{Fore.RED}[QUITTING]...{Fore.WHITE}")
		return BROKEN_PIPE
...
def login(i=0):
	...
	if cred_response != LOGIN_SUCCESSFUL:
		if i == 2:
			...
			print(f"{Fore.RED}[ERROR]Too many failed login attempts")
			return False
		return login(i+1)
	return True
	...
	if server_msg_list != msg_list:
		for i in range(len(server_msg_list)):
			if i >= len(msg_list) or server_msg_list[i] != msg_list[i]:
				print(f"{Fore.BLUE}{server_msg_list[i][0]} {server_msg_list[i][1]}{Fore.WHITE}")   
